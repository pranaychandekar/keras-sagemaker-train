...
# ML Keras Training with Amazon SageMaker    
 ...
 Hi! I am Pranay Chandekar and in this tutorial, we will work on building and training our custom Keras model in Amazon SageMaker.     
    
For a better understanding, I would recommend going through the below links before starting the tutorial:    
  
 - [Cloud Services for Machine Learning](https://www.linkedin.com/pulse/cloud-services-machine-learning-pranay-chandekar/)  
 - [Custom Keras based Machine Learning Model Training with Amazon SageMaker](https://medium.com/@pranaychandekar/custom-keras-based-machine-learning-model-training-with-amazon-sagemaker-4f536454f6e7)  
  
    
    
The tutorial is divided into 6 parts. From Part 1 to Part 5, we will implement the project on a CPU. In Part 6, we will see how we can now tweak this project to use a GPU. In this repository, I have defined a convention for project structure, filenames etc. to ensure reusability. You can clone this repository and perform minimal editing to get going with your own training logic. **Feel free to fork it or raise a pull request to make it more generic.** Lets dive into our tutorial.    
...
```bash
cd ~/SageMaker/ 
git clone https://github.com/pranaychandekar/keras-sagemaker-train.git
``` 
...
```bash  
docker build --network=host -t keras-sagemaker-train:cpu -f Dockerfile.cpu .
``` 
...
We now have a saved model '***demo.h5***' in the '***model***' directory.    
...
```bash  
cd ~/SageMaker/keras-sagemaker-train/local_test/ ./train_local_gpu.sh keras-sagemaker-train:gpu 
``` 
...
***Congratulations for a successful GPU training job run!*** <p align="center">    
